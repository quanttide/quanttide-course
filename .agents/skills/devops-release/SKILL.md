...
gh release create <version> \
  --title "v<version>" \
  --notes "$(sed -n "/^## \[${VERSION#v}\]/,/^## \[/p" CHANGELOG.md | sed '1d;$d')" \
  --repo quanttide/quanttide-course

# 9. 验证 Release
gh release view <version> --repo quanttide/quanttide-course
```
...
# 清理预发布版本
gh release delete vX.Y.Z-rc.1 --repo quanttide/quanttide-course --yes
```
...
✓ Release vX.Y.Z 创建成功
  标签: vX.Y.Z
  URL: https://github.com/quanttide/quanttide-course/releases/tag/vX.Y.Z
  提交: <SHA>
```
